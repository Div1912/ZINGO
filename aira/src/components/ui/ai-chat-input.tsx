...
function ModelIcon({ model, className }: { model: string; className?: string }) {
  ...
  // Local MRPL Model SVGs
  if (model.includes("Qwen") || model.includes("AIRA") || model.includes("Coder") || model.includes("Auto")) {
    return (
      <svg className={cn("size-3.5 text-content-primary shrink-0", className)} viewBox="0 0 100 100" fill="none" stroke="currentColor" strokeWidth="12">
        <polygon points="50,8 88,30 88,72 50,94 12,72 12,30" />
      </svg>
    );
  }
  ...
  return (
    <img 
      ...
      onError={(e) => {
        // Fallback SVG if external CDN icon is offline in air-gapped intranet
...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(
  (
    ...
    // Updates height on value/text change
    useEffect(() => {
    ...
    return (
      <>
        ...
        <div
          ...
          onBlur={handleBlur}
          className={cn("relative flex flex-col w-full transition-all", className)}
          style={{
        ...
        >
          ...
          <div
            ...
            className={cn(
              "relative w-full border border-border bg-card shadow-sm focus-within:border-ring/40 focus-within:ring-1 focus-within:ring-ring/20 hover:border-border/80 z-10 transition-all",
              expanded ? "cursor-text" : "cursor-default",
              isGenerating && "border-border-strong shadow-md"
            )}
          >
            {/* Animate border with BorderBeam */}
            {enableBorderBeam && (
              <BorderBeam
                duration={isGenerating ? 3.5 : isRecording ? 5 : 8}
                colorFrom={isRecording ? "#ef4444" : isGenerating ? "#38bdf8" : "#38bdf8"}
                colorTo={isRecording ? "#f59e0b" : isGenerating ? "#34d399" : "#818cf8"}
                borderWidth={1.5}
              />
            )}
            ...
            >
              <div className="relative">
                <button
                  ...
                  className={cn(
                    "group flex items-center gap-1 rounded-full px-2 py-1 text-foreground/70 transition-all duration-200 outline-none hover:bg-muted hover:text-foreground cursor-default",
                    isModelSelectOpen ? "bg-muted text-foreground" : ""
                  )}
                ...
                <div
                  ...
                  className={cn(
                    "absolute bottom-full left-0 mb-2.5 z-50 w-52 rounded-2xl border border-border bg-surface p-1 shadow-xl backdrop-blur-md flex flex-col gap-0.5 transition-all duration-400 cursor-default",
                    isModelSelectOpen
              ...
              <button
                ...
                className="ml-auto flex size-7 items-center justify-center rounded-full text-foreground/70 transition-all duration-200 hover:bg-muted hover:text-foreground outline-none cursor-default disabled:opacity-40 disabled:pointer-events-none"
