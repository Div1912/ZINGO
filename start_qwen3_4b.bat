@echo off
title ZINGO Fast Synthesis Node Launcher (Qwen3-4B)
echo ===================================================
echo   Starting ZINGO Fast Synthesis Node & Ngrok Tunnel
...
if "%ERRORLEVEL%"=="0" (
    ...
    if exist "%~dp0ngrok.exe" (
        start "ZINGO Qwen3-4B Tunnel" cmd /k "cd /d %~dp0 && ngrok.exe http --domain %PERMANENT_DOMAIN% --host-header=rewrite 11434"
    ) else (
        where ngrok >nul 2>&1
        if "%ERRORLEVEL%"=="0" (
...
echo.
echo ===================================================
echo   Ready! Qwen3-4B Fast Synthesis Node is LIVE:
